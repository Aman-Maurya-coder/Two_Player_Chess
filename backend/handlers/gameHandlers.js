    ...
    joinGame(socket) {
        socket.on("joinGame", ({ roomId, playerId }) => {
            ...
            if (gameId === undefined) {
                ...
                socket.emit(
                    "gameFull",
                    "Game is already full. Please try joining another game."
                ); // Notify the client if game is full
                return;
                ...
                if (
                    ...
                    if (
                        this.games[roomId]["roomPlayers"]["white"] ===
                            playerId ||
                        ...
                        socket.emit(
                            "roomFull",
                            "You are not part of this game room. Please try joining another game."
                        );
                        return;
                    }
                } else {
                    ...
                    if (this.games[roomId]["roomPlayers"]["white"] === null) {
                        ...
                        this.games[roomId]["roomPlayers"]["black"] === null
                    ) {
                        this.games[roomId]["roomPlayers"]["black"] = playerId;
                ...
                socket
                    ...
                    .emit("playerJoinedRoom", {
                        gameId: roomId,
                        gameStatus: this.games[roomId]["gameStatus"],
                    });
                socket.emit("roomJoined", {
                    gameId: roomId,
                    gameStatus: this.games[roomId]["roomStatus"],
                    playerSide:
                        this.games[roomId]["roomPlayers"]["white"] === playerId
                            ? "white"
                            : "black",
                    timeControl: this.games[roomId]["timer"] * 60 * 1000,
                }); // Notify the client that the player has joined the room
            } else if (
                gameId["gameStatus"] === "waiting for reconnection" &&
                this.players[playerId]["playerStatus"] ===
                    "disconnected from game"
            ) {
                if (
                ...
                global.io
                    .in(roomId)
                    .emit("playerJoinedRoom", { gameId: roomId }); // Notify the client that the player has joined the room
            }
    ...
    makeMove(socket) {
        socket.on("move", ({ move, gameId }) => {
            ...
            global.io.in(gameId).emit("moveMade", {
                ...
                currentTurn:
                    currentGameData.game.turn() === "w" ? "white" : "black",
            });
            ...
            if (currentGameData.game.isGameOver()) {
                ...
                else if (currentGameData.game.isStalemate())
                    reason = "stalemate";
                else if (currentGameData.game.isDraw()) reason = "draw";
                else if (currentGameData.game.isInsufficientMaterial())
                    reason = "insufficient material";
                else if (currentGameData.game.isDrawByFiftyMoves())
                    reason = "draw by fifty moves";
                else if (currentGameData.game.isThreefoldRepetition())
    ...
    onAbort(socket) {
        socket.on("abort", ({ playerId, gameId }) => {
            if (this.games[gameId] !== undefined) {
                ...
                const winner =
                    this.games[gameId]["roomPlayers"]["white"] === playerId
                        ? this.games[gameId]["roomPlayers"]["black"]
                        : this.games[gameId]["roomPlayers"]["white"];
                global.io
                    .in(gameId)
                    .emit("gameAborted", {
                        winner: winner,
    ...
    onDrawAccept(socket) {
        socket.on("drawAccepted", ({ gameId }) => {
            if (this.games[gameId] !== undefined) {
                ...
                global.io
                    .in(gameId)
                    .emit("gameDraw", { reason: "draw", gameStatus: "draw" });
            }
        });
    }
